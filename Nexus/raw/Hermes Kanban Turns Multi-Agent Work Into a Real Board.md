...
created: 2026-05-31
description: "Most “multi-agent” workflows are still too invisible.You ask one agent to split work across subagents. They run somewhere inside the process..."
tags:
  - "clippings"
---
![Imatge](https://pbs.twimg.com/media/HHgWZeUaUAAzq7h?format=jpg&name=large)
...
Hermes already has delegate\_task for fork-and-join subagent work. Kanban solves a different problem.

**delegate\_task** is a function call.

Kanban is a durable work queue plus state machine.

Takeaway: Use delegate\_task when the parent needs a short answer back. Use Kanban when work needs persistence, retry, visibility, human input, or multiple roles over time.
...
That environment enables a dedicated kanban\_\* toolset inside the worker’s model schema:
...
That matters because the worker’s terminal backend might be local, Docker, SSH, Modal, Singularity, or something else. The Kanban tools run in the agent’s own Python process and route through the same kanban\_db layer as the CLI and dashboard.

Even better:

Normal Hermes sessions do not carry these tools. The kanban\_\* tools only appear when HERMES\_KANBAN\_TASK is set.
...
![Imatge](https://pbs.twimg.com/media/HHgtDAPaQAAzd-p?format=jpg&name=large)

or

researcher A + researcher B → analyst → writer → editor

![Imatge](https://pbs.twimg.com/media/HHgtRF3bYAAlU1A?format=jpg&name=large)
