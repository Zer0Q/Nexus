...
created: 2026-06-02
description: "A non-technical leader's guide to The Software Factory, how it works, why it's in vogue, and how to get your company to operate one.In 2023,..."
tags:
  - "clippings"
summary:
---
![Imatge](https://pbs.twimg.com/media/HJwg4puWMAA2aYW?format=jpg&name=large)
...
But when some companies are attributing 90% of their production software to AI (read: [@AnthropicAI](https://x.com/@AnthropicAI)) and best-in-class ICs are matching the output of a 20-person pre-AI engineering org, you need to care.

Reality is, most companies are nowhere close to having a true Software Factory.

I run a company ([@tenex\_labs](https://x.com/@tenex_labs)) where one of the things we now do is help enterprises build these. We set up the infrastructure and tooling, then run the assessment, training, and change management to get an engineering org ready for an entirely new way of building software. So I look at a lot of these orgs. And the gap between what CEOs think they have and what they actually have is enormous.
...
The most extreme version I've seen comes from a system [@simonw](https://x.com/@simonw) wrote about, built by a company called StrongDM. Two guiding principles:

> "Code must not be written by humans." "Code must not be reviewed by humans."

([Simon Willison on X](https://x.com/simonw/status/2020161285376082326).)
...
**1) The proof points got undeniable.** [@ryancarson](https://x.com/@ryancarson) now runs what he calls a "Code Factory." Agents write the code, review it, run the tests, triage the errors, and watch production, while he sets the guardrails. He's shipped over a thousand pull requests this way. Basically a one-person software company. Meanwhile Anthropic says 90%+ of its code is AI-written, Google says 75% of new code is AI-generated (up from 25% a year ago), and at OpenAI, 95% of engineers use their internal agent. The ones who lean in open 70% more pull requests than their peers ([Lenny Rachitsky on X](https://x.com/lennysan/status/2022121036364529702)).

**2) Big names are giving it oxygen.** Microsoft is now pitching an "Agent Factory" as the new way software gets built. When the giants adopt the language, it goes mainstream fast. [@chamath](https://x.com/@chamath) talks frequently about the Software Factory his team at 8090 is building. [@garrytan](https://x.com/@garrytan) hits on this factory concept with GStack, GBrain, and his newest article titled "Stop building Foxconn factories for your agents."
...
1. **What percentage of our pull requests ship without a human reviewing them?** If it's zero, you're Level 1.
2. **When we fix a bug or build a small feature, who writes the first version: a person, or a person directing an agent?** (A person = Level 0 or 1. A person directing an agent = Level 2 and up.)
3. **Can an agent open a pull request on its own, or does a human write every change by hand?** (This is the line between Level 1 and Level 2.)
4. **Do we have rules that let low-risk changes ship automatically while escalating the risky ones, and who set those rules?** (This is the heart of Level 3.)
5. **When something breaks in production, what notices first: a person or a system? And what triages it?** (Humans noticing = early. A system catching and triaging it = Level 3.)
6. **Do our tests run automatically on every change, and can the system block a bad one without a human stepping in?** (Tells you if the quality gate is real or just a person clicking approve.)
7. **If an agent ships something wrong, how fast can we catch it and undo it?** (Your safety net decides how far up the ladder you can safely climb.)
8. **Can our AI tools securely see our own codebase, docs, and systems, or just generic public knowledge?** (Generic = you're capped until you fix it.)
9. **Do we have an internal platform our agents plug into, or is every engineer wiring up their own tools?** (Per DORA, this is the single biggest differentiator.)
10. **If I took every engineer's AI tools away tomorrow, would our process change, or just our speed?** (Only speed = Level 1 with better autocomplete. The process would break = you've actually industrialized.)
11. **Where's our bottleneck right now: writing the code, or reviewing and shipping it?** (Still writing = early. Moved downstream = mature. That shift is the whole point.)
...
Because Ryan Carson's right, engineering is just the first factory. As he put it: "Get your Code Factory set up because the next step is the Company Factory." ([@ryancarson](https://x.com/ryancarson/status/2024436111964004461).)
...
- **Ryan Carson** ([@ryancarson](https://x.com/ryancarson)). The clearest real-world example of a working Code Factory. He publishes his actual setup, not theory.
- **Simon Willison** ([@simonw](https://x.com/simonw)). The most credible, hype-free voice on what agentic coding can and can't do.
- **Peter Steinberger** ([@steipete](https://x.com/@steipete)). Creator of [@openclaw](https://x.com/@openclaw). Constantly pushing the boundaries of what's possible with agentic coding.
- **Lenny Rachitsky** ([@lennysan](https://x.com/lennysan)). Best at translating what frontier orgs (OpenAI, Anthropic) are doing into business language.
- **Santiago** ([@svpino](https://x.com/svpino)). Your designated skeptic. He'll call BS on the "90% of code" claims and keep you honest.
- **Andrej Karpathy** ([@karpathy](https://x.com/karpathy)). For the deep "why," which work AI can and can't absorb, and why.
- **The DORA report** ([dora.dev](https://dora.dev/dora-report-2025/)). Not a person, but the best evidence-based read on what separates teams that win with AI.
...
- 5 levels of the Software Factory Ladder: Artisan (Everything is a human), Assisted (glorified autocorrect), Assisted (agent drives execution), Supervised Factory (human absracted other than direction/guardrails & exceptional handling), Autonomous Factory (agentic nirvana)
