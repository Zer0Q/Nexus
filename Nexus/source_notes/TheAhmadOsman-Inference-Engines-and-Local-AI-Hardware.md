...
- [[KV-Cache-Growth]] -- KV cache grows with batch size and context length
- [[Prefill-vs-Decode]] -- two phases with different resource profiles
- [[PagedAttention]] -- KV cache memory management via block partitioning
- [[Continuous-Batching]] -- scheduler quality determines throughput
- [[Disaggregated-Prefill-Decode]] -- separating compute and memory phases
- [[Interconnect-Bottleneck]] -- communication cost dominates multi-GPU
- [[Quantization-Format-Portability]] -- formats are engine-specific, not interchangeable
- [[Speculative-Decoding]] -- draft cheap tokens, verify in parallel
- [[Unified-Memory-Tradeoff]] -- capacity superpower with bandwidth tradeoffs
- [[Hardware-First-Engine-Selection]] -- engine follows hardware strategy, not vice versa
- [[Benchmarking-Workload-Shape]] -- proper benchmarking requires full context
